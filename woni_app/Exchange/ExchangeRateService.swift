...
/// 환율 조회 API. 백엔드 `/api/exchange-rates` 계약에 대응한다.
struct ExchangeRateService {
    ...
    /// 특정 날짜의 전체 통화 환율. `GET /api/exchange-rates?date=yyyy-MM-dd`
    func fetchRates(on date: Date) async throws -> [ExchangeRate] {
        ...
        let dtos: [ExchangeRateDTO] = try await client.get("/api/exchange-rates", query: query)
    ...
    func fetchRate(for currency: CurrencyCode, on date: Date? = nil) async throws -> ExchangeRate {
        ...
        let dto: ExchangeRateDTO = try await client.get(
            "/api/exchange-rates/\(currency.rawValue)",
            query: query
        )
        return dto.toDomain()
    }
